...
public class CreditAccount implements Account {
	...
	public static final int default_limit = 100_00; // in cents
	...
	public static CreditAccount newOfBalanceOwner(int balance, Person owner) {
		return new CreditAccount(CreditAccount.default_limit, balance, owner);
	}

	// object methods

	public int deposit(int amount) { // amount in cents
		return this.balance = Math.addExact(this.balance, CreditAccount.requirePositive(amount)); // overflow is possible
	}

	public int withdraw(int amount) { // amount in cents
		int newBalance = Math.subtractExact(this.balance,CreditAccount.requirePositive(amount)); // balance can be negative, overflow is possible!
